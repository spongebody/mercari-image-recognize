...
```
{
  ...
  "alternatives": [
    { "target_path": "CD・DVD・ブルーレイ > CD > その他", "category_id": "002" },
    { "target_path": "CD・DVD・ブルーレイ > CD > K-POP・アジア", "category_id": "001" }
  ],
  "brand_name": "Nintendo"
}
```
...
| `best_category_id` | string | 首选分类的 ID（等同于 `categories[0].id`） |
| `alternatives` | array[object] | 备选分类路径及其 ID |
| `alternatives[].target_path` | string | 备选分类路径 |
| `alternatives[].category_id` | string | 备选分类 ID |
